...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray,
        ...
        """
        m = X.shape[0]
        return -(1 / m) * np.sum(y * (X @ self.weights) - np.log(1 + np.exp(X
                                                                            @ self.weights_)))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray,
        ...
        """
        m = X.shape[0]
        return (-1 / m) * (y @ X - X.T @ (1 / (1 + np.exp(-self.weights @
                                                          X.T))))
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        jac = self.regularization_module_.compute_jacobian()
        if self.include_intercept_:
            jac = np.concatenate((np.ones(1), jac))
